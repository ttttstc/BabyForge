...
const EMAIL_HERO_PATH = '/assets/login/login-hero-mobile.png'
...
function renderEmailCore({ familyName, babyName, actorName, action, category, context, rows, url, settingsUrl, isPhoto = false }) {
  const family = familyName || '你们家庭'
  const baby = babyLabel(babyName)
  const label = categoryLabel(category)
  const actionText = actionCopy(action, category)
