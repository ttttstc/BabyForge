...
function renderEmail({ familyName, babyName, actorName, action, category, context, rows, url, heroUrl, isPhoto = false }) {
...
function photoMessage({ householdName, babyName, actorName, action, url, heroUrl }) {
...
export async function sendUpdateNotifications({ env, householdId, actorUserId, actorName, babyName, action, previous = null, next = null, photo = false, url, heroUrl }) {
  const targets = await recipients(env, householdId, actorUserId)
  if (!targets.length) return
  const householdName = targets[0].householdName || '你们家庭'
  const message = photo
    ? photoMessage({ householdName, babyName, actorName, action, url, heroUrl })
    : eventMessage({ householdName, babyName, actorName, action, previous, next, url, heroUrl })
