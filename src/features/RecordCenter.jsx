...
import { createEvaluatedGrowthMeasurement, GROWTH_SOURCES, growthSourceLabel } from '../domain/growth.js'
...
export function RecordCenter({ state, commitState, onClear, onLogout, readOnly = false, role = 'admin' }) {
  ...
  const returnTo = (() => {
    const value = new URLSearchParams(window.location.hash.split('?')[1] || '').get('returnTo')
  ...
  return (
    <main className="record-center-page">
      ...
      <div className="record-center-shell">
        ...
        {activePanel === 'growth' && <RecordSubsection title={isEnglish ? 'Growth measurement' : '成长测量'} onClose={() => setActivePanel(null)}><GrowthPanel history={state.growthMeasurements} locale={locale} readOnly={readOnly} onSave={saveGrowth} /></RecordSubsection>}
...
function GrowthPanel({ history, locale, readOnly, onSave }) {
  const isEnglish = locale === 'en-US'
  ...
  async function submit(event) { event.preventDefault(); if (!value.trim()) return; setSaving(true); try { await onSave({ type, value, measuredAt: date, unit: definition.unit, source }); setValue('') } finally { setSaving(false) } }
