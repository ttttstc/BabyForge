...
import { Activity, ArrowLeft, Baby, Check, ChevronRight, ClipboardList, Clock3, FileText, HeartPulse, Pill, Plus, Save, ShieldCheck, Thermometer, Utensils, X } from 'lucide-react'
...
export function RecordCenter({ state, setState, onClear, onLogout, readOnly = false, role = 'admin' }) {
  ...
  function updateProfile(profile) {
    setEntryError('')
    return Promise.resolve(setState((current) => {
      ...
      const profileChanged = ['birthDate', 'gestationalWeeks', 'gestationalDays', 'growthAgeBasis', 'birthMultiplicity'].some((key) => current.baby?.[key] !== nextBaby[key])
      ...
      showSaved(isEnglish ? 'Baby profile saved' : '基础信息已保存')
      return true
    }).catch((error) => {
      setEntryError(error?.message || (isEnglish ? 'Save failed. Retry.' : '保存失败，请重试。'))
      throw error
  ...
  function createConcern(input) {
    ...
    return recordEvent({ category: input.topicId, payload: { concernId: concern.id, supportTopic: input.topicId, supportTitle: topic.title, facts: input.facts || [], notes: input.notes || '', plan: input.plan || null } }, isEnglish ? 'Follow-up saved' : '关注事项已保存').then(() => setState((current) => ({ ...current, concerns: [...(current.concerns || []), concern] })).then(() => concern))
  }

  function resolveConcern(concernId) {
    const concern = state.concerns.find((item) => item.id === concernId)
    return recordEvent({ category: 'care_action', payload: { concernId, supportStatus: 'closed', supportTitle: concern?.title || '' } }, isEnglish ? 'Follow-up closed' : '关注事项已结束').then(() => setState((current) => ({ ...current, concerns: current.concerns.map((item) => item.id === concernId ? { ...item, status: 'closed', updatedAt: new Date().toISOString() } : item) })))
  }
...
function moreTitle(id) {
  return { growth: 'Growth measurements', care: 'Care actions', concern: 'Follow-up topics', professional: 'Professional record' }[id]
}

function moreDetail(id) {
  return { growth: 'Weight, length, and head circumference', care: 'Daily actions, errands, and milestones', concern: 'Open, track, and close a topic', professional: 'Clinician conclusions and instructions' }[id]
}
