...
function initialSelection(defaultView = 'diseases') {
  ...
  return {
    view: params.get('view') === 'organs' || defaultView === 'organs' ? 'organs' : 'diseases',
...
function syncHash(view, diseaseId, organId) {
  ...
  const route = view === 'organs' ? ROUTES.healthOrgans : ROUTES.healthDiseases
...
export function PediatricDiseasesView({ route = ROUTES.healthDiseases, state, setState, onClear, onLogout, readOnly = false, role = 'admin' }) {
  const locale = state.preferences.locale
  const initial = useMemo(() => initialSelection(route === ROUTES.healthOrgans ? 'organs' : 'diseases'), [route])
    ...
    <div className={`disease-hub ${view === 'organs' ? 'disease-hub-organ-mode' : ''}`}>
      ...
      {view === 'diseases' ? <>
        ...
        <section className="disease-selected" ref={detailRef} tabIndex="-1">
          <header className="disease-selected-header"><div><p className="eyebrow">{localized(disease.category, locale)} · {localized(disease.ageRange.label, locale)}</p><h2>{localized(disease.name, locale)}</h2><p>{localized(disease.shortDefinition, locale)}</p></div><button onClick={() => navigate(buildNaibaRoute({ skill: 'triage_and_preassessment', unit: disease.escalationRuleRef.split(':').pop(), contentType: 'disease', contentId: disease.id, returnTo: `${ROUTES.healthDiseases}?disease=${disease.id}` }))}><ShieldCheck size={16} /><span><b>{isEnglish ? 'Assess my child\'s current state' : '判断宝宝当前状态'}</b><small>{isEnglish ? 'Open the governed decision flow' : '进入统一健康预评估'}</small></span><ArrowRight size={16} /></button></header>
      ...
      </> : <OrganLearning organ={organ} setOrganId={setOrganId} relatedDiseases={relatedDiseases} selectDisease={selectDisease} locale={locale} performanceMode={state.preferences.performanceMode} />}
    </div>
  </main>
}
...
function OrganLearning({ organ, setOrganId, relatedDiseases, selectDisease, locale, performanceMode }) {
  const isEnglish = locale === 'en-US'
  ...
  return <div className="pediatric-workspace organ-learning-workspace">
    <aside className="pediatric-library" aria-label={isEnglish ? 'Organ library' : '器官资料库'}>
      <div className="pediatric-panel-heading"><span>{isEnglish ? 'Explore organ models' : '探索器官模型'}</span><small>{ANATOMY_RESOURCES.length} {isEnglish ? 'models · swipe' : '个模型 · 横向滑动'}</small><BookOpen size={16} /></div>
