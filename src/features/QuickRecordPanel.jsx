...
export function QuickRecordPanel({ locale = 'zh-CN', onRecord, onConcern, readOnly = false }) {
  ...
  function record(item) {
    ...
    onRecord?.({ type: item.type, payload: item.kind ? { kind: item.kind } : { mode: 'breastfeeding' } })
  }

  function submitBottle(event) {
    event.preventDefault()
    if (!amount.trim()) return
    onRecord?.({ type: 'bottle_feeding', payload: { amountMl: Number(amount), unit: 'mL' } })
    setAmount('')
    setBottleOpen(false)
  }
