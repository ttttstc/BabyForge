...
export function LoginView({ locale = 'zh-CN', onLocaleChange, onLogin, onRegister, onGoogleLogin, onForgotPassword, onResetPassword, onResetComplete, onSetAccountPassword, passwordSetupMode = false, accountEmail = '', onResendVerification, resetMode = false, resetToken = '', resetError = '', error = '', noProfile = false }) {
  const copy = getCopy(locale)
  const isEnglish = locale === 'en-US'
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [confirmPassword, setConfirmPassword] = useState('')
  const [mode, setMode] = useState(passwordSetupMode ? 'passwordSetup' : resetMode ? 'reset' : 'login')
  ...
  const headings = {
    ...
    passwordSetup: isEnglish ? 'Secure your Google account' : '为 Google 账号设置登录密码',
  }
  const descriptions = {
    login: noProfile
    ...
    passwordSetup: isEnglish ? `Set a password for ${accountEmail}. Next time you can use Google or email and password.` : `为 ${accountEmail} 设置密码。下次可使用 Google 或邮箱密码登录。`,
  }
    ...
    <main className="login-shell">
      ...
      <section className="login-card">
        ...
        <div className="login-heading">
          <p className="eyebrow">{mode === 'login' ? (isEnglish ? 'Welcome back' : '欢迎回来') : mode === 'register' ? (isEnglish ? 'Start with less' : '轻松开始') : mode === 'passwordSetup' ? (isEnglish ? 'One last step' : '最后一步') : (isEnglish ? 'Account recovery' : '账号恢复')}</p>
          <h2>{headings[mode]}</h2>
        ...
        {(mode === 'login' || mode === 'register' || mode === 'passwordSetup') && <>
          <div className="login-divider"><span>{isEnglish ? 'or' : '或'}</span></div>
          <button className="login-google" type="button" disabled={busy} onClick={async () => { setBusy(true); setNotice(''); try { await onGoogleLogin?.() } catch (googleError) { setNotice(googleError.message || (isEnglish ? 'Google sign-in is unavailable.' : 'Google 登录暂时不可用。')) } finally { setBusy(false) } }}><GoogleMark />{mode === 'passwordSetup' ? (isEnglish ? 'Verify again with Google' : '使用 Google 重新验证') : (isEnglish ? 'Continue with Google' : '使用 Google 账号继续')}</button>
        </>}

        {mode !== 'login' && mode !== 'passwordSetup' && <div className="login-links login-links-single"><button type="button" onClick={() => switchMode('login')}>{isEnglish ? 'Back to sign in' : '返回登录'}</button></div>}
