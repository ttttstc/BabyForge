...
import { Baby, BookOpen, CalendarDays, Globe2, HeartPulse, ListChecks, LockKeyhole, LogIn, Mail } from 'lucide-react'
    ...
    <main className="login-shell">
      ...
      <section className="login-card">
        ...
        <form className="login-form" onSubmit={submit}>
          {(mode === 'register' || mode === 'login') && <label htmlFor="auth-email"><span>{isEnglish ? 'Email' : '邮箱'}</span><div className="login-input"><Mail size={17} /><input id="auth-email" type={mode === 'register' ? 'email' : 'text'} inputMode="email" autoComplete="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" /></div></label>}
