...
import { evaluateSupport, SUPPORT_TOPICS, topicById, URGENT_FACTS } from '../domain/healthSupport.js'
...
function SupportPlan({ locale, plan, onClose }) {
  const isEnglish = locale === 'en-US'
  return <div className={`support-plan ${plan.actionLevel}`}><header><strong>{isEnglish ? 'Next steps' : '下一步'}</strong><button type="button" onClick={onClose} aria-label={isEnglish ? 'Close next steps' : '关闭下一步'}><X size={15} /></button></header><p><b>{isEnglish ? 'Now' : '现在'}：</b>{text(plan.action, locale)}</p><p><b>{isEnglish ? 'Recheck' : '复查'}：</b>{text(plan.recheck, locale)}</p><p><b>{isEnglish ? 'Escalate' : '需要升级'}：</b>{text(plan.escalation, locale)}</p><small>{plan.source}</small></div>
}
