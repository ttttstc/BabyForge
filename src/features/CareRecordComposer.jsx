...
import { assertCareRecordInput } from '../domain/careEvents.js'
...
export function P0RecordComposer({ type, locale = 'zh-CN', readOnly = false, initialEvent = null, recentGrowth = {}, onSave, onCancel }) {
  ...
  async function submit(event) {
    ...
    if (type === 'temperature') {
      ...
      return save({ kind: hasValue ? 'measurement' : 'caregiver_observation', category: hasValue ? 'temperature' : 'temperature_observation', occurredAt: isoValue(form.occurredAt), payload: hasValue ? { value: Number(form.value), unit: form.unit, method: form.method } : { method: form.method } }, isEnglish ? 'Temperature saved' : '体温已保存')
    }
    const definition = GROWTH_TYPES.find((item) => item.id === form.type)
    return save({ kind: 'measurement', category: 'growth_measurement', occurredAt: `${form.measuredAt}T12:00:00`, payload: { type: form.type, value: String(form.value).trim(), unit: definition.unit, measuredAt: form.measuredAt, source: form.source, ageBasis: 'chronological' } }, isEnglish ? 'Growth saved' : '成长测量已保存')
  }
