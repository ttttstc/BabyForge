...
import { ArrowRight, Baby, Check, CircleHelp, ExternalLink, Info, LineChart, ListChecks, ShieldCheck } from 'lucide-react'
import { getAgeDays, getStage } from '../domain/baby.js'
import { getAdminTasks, getStageMilestones, GROWTH_TYPES } from '../domain/carePlan.js'
...
function GrowthSubnav({ route, locale }) {
  const isEnglish = locale === 'en-US'
  const items = [[ROUTES.growth, isEnglish ? 'Overview' : '总览'], [ROUTES.growthChart, isEnglish ? 'National chart' : '国家曲线'], [ROUTES.growthStage, isEnglish ? 'Stage guide' : '阶段指南'], [ROUTES.growthHistory, isEnglish ? 'History' : '历史记录']]
...
function GrowthOverview({ locale, stage, ageContext, content, metricCards, dataIssues, milestones, onRecord, onOpenRecord, onOpenChart, onOpenStage, onOpenHistory }) {
  const isEnglish = locale === 'en-US'
  return <div className="growth-overview">
    ...
    <section className="growth-board-card"><header className="growth-card-heading"><div><p className="eyebrow">{isEnglish ? 'Baby development highlights' : '宝宝发展重点'}</p><h2>{isEnglish ? 'What to notice' : '这阶段可以留意什么'}</h2></div><CircleHelp size={19} /></header><p className="growth-card-note">{isEnglish ? 'Keep concrete observations. These highlights are not a screening score.' : '只记录具体场景。这些重点不是发育筛查评分。'}</p><div className="growth-highlight-list">{content.babyHighlights.slice(0, 3).map((item) => <article key={item.id}><strong>{item.title}</strong><p>{item.detail}</p><small>{item.caution}</small></article>)}</div></section>
    <section className="growth-board-card growth-activity-card"><header className="growth-card-heading"><div><p className="eyebrow">{isEnglish ? 'Optional activity' : '可选亲子活动'}</p><h2>{content.recommendedActivities[0]?.title}</h2></div><ShieldCheck size={19} /></header><p>{content.recommendedActivities[0]?.detail}</p><button type="button" className="secondary-button compact" onClick={onOpenRecord}>{isEnglish ? 'Record the baby’s response' : '去记录中心记录实际反应'}</button></section>
    {dataIssues.length > 0 && <DataIssueCard locale={locale} issues={dataIssues} onOpenHistory={onOpenHistory} />}
  </div>
}
...
function GrowthChartPage({ locale, baby, measurements, metric, onMetricChange }) {
  ...
  return <div className="growth-detail-page"><section className="growth-detail-lede"><p className="eyebrow">{isEnglish ? 'Chinese national growth standard' : '中国国家儿童生长标准曲线'}</p><h2>{isEnglish ? 'Track the baby against the official reference' : '把宝宝轨迹放在国家标准上看'}</h2><p>{isEnglish ? 'The chart draws P3, P10, P25, P50, P75, P90, and P97. Your measurements remain the strongest visual line.' : '曲线完整绘制 P3、P10、P25、P50、P75、P90、P97；宝宝自己的测量轨迹始终最突出。'}</p></section><section className="growth-board-card growth-chart-card"><header className="growth-card-heading"><div className="growth-switcher">{GROWTH_TYPES.map((item) => <button type="button" key={item.id} className={metric === item.id ? 'active' : ''} onClick={() => onMetricChange(item.id)}>{localized(item.label, locale)}</button>)}</div><div className="growth-range-switcher"><button type="button" className={range === 3 ? 'active' : ''} onClick={() => setRange(3)}>0–3 {isEnglish ? 'mo' : '个月'}</button><button type="button" className={range === 12 ? 'active' : ''} onClick={() => setRange(12)}>0–12 {isEnglish ? 'mo' : '个月'}</button></div></header><NationalGrowthChart locale={locale} model={model} /><footer className="growth-chart-footer"><span>{isEnglish ? `${model.standard.id} · version ${model.standard.version}` : `${model.standard.id} · 版本 ${model.standard.version}`}</span><a href={model.standard.sourceUrl} target="_blank" rel="noreferrer">{isEnglish ? 'Official source' : '国家卫健委标准原文'}<ExternalLink size={13} /></a></footer></section></div>
}
...
function NationalGrowthChart({ locale, model }) {
  ...
  if (!allValues.length) return <div className="growth-chart-empty"><LineChart size={26} /><p>{isEnglish ? 'Set sex and add a valid measurement to draw the national curve.' : '设置宝宝性别并录入有效测量后，才能绘制国家标准曲线。'}</p></div>
