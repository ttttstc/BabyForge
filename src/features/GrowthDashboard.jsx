...
import { ArrowLeft, ArrowRight, Baby, CircleHelp, ExternalLink, Info, LineChart, ListChecks, Maximize2, MessageCircle, ShieldCheck } from 'lucide-react'
...
export function GrowthDashboard({ route = ROUTES.growth, state, setState, onClear, onLogout, readOnly = false, role = 'admin', cloudMode = false }) {
  ...
  const openRecord = (panel = 'growth') => navigate(recordRoute(panel, route))
  const openChart = (nextMetric = activeMetric) => navigate(`${ROUTES.growthChart}?metric=${encodeURIComponent(nextMetric)}`)
...
function GrowthOverview({ locale, stage, ageContext, content, metricCards, activeMetric, baby, measurements, summary, parentActions, onOpenRecord, onRecord, onOpenChart, onOpenStage, onOpenHistory, onOpenAi }) {
  ...
  return <div className="growth-overview growth-dashboard-order">
    ...
    <section className="growth-board-card growth-measurement-board"><header className="growth-card-heading"><div><p className="eyebrow">{isEnglish ? 'Latest facts' : '最近成长事实'}</p><h2>{isEnglish ? 'Three measurements, one dashboard' : '三项测量，一个看板'}</h2></div><button type="button" className="text-button" onClick={onRecord}>{isEnglish ? 'Add a measurement' : '补录一次测量'}<ArrowRight size={15} /></button></header><p className="growth-card-note">{isEnglish ? 'Cards show the latest fact first. A reference result appears only when the official package applies.' : '卡片先展示最近事实；只有官方标准适用时才显示参考结果。'}</p><div className="growth-metric-grid">{metricCards.map((item) => <MetricCard key={item.id} item={item} locale={locale} onOpen={() => onOpenChart(item.id)} />)}</div></section>
    ...
    <RecentGrowthRecords locale={locale} evaluations={measurements.map((item) => ({ ...item, evaluation: evaluateGrowthMeasurement(item, baby, measurements) }))} onOpenHistory={onOpenHistory} />
  </div>
}

function MetricCard({ item, locale, onOpen }) {
  const isEnglish = locale === 'en-US'
  const latest = item.latest
  const evaluation = latest?.evaluation
  return <article className={`growth-metric-card ${latest?.conflicted ? 'conflicted' : ''}`}><button type="button" className="growth-metric-card-button" onClick={onOpen}><header><span>{localized(item.label, locale)}</span><span>{item.unit}</span></header><strong>{latest ? `${latest.value} ${latest.unit}` : '—'}</strong><small>{latest ? formatDate(latest.measuredAt, locale) : (isEnglish ? 'No record yet' : '暂无记录')}</small>{latest?.conflicted ? <div className="growth-reference-result muted"><b>{isEnglish ? 'Verify same-day values' : '同日数值需核对'}</b><span>{isEnglish ? 'Change is not calculated.' : '暂不计算变化。'}</span></div> : evaluation?.dataQuality === 'sufficient' ? <div className="growth-reference-result"><b>{standardResultLabel(evaluation, locale)}</b><span>{standardResultDetail(evaluation, locale)}</span></div> : <div className="growth-reference-result muted"><b>{isEnglish ? 'Fact only' : '先展示事实'}</b><span>{evaluation?.limitations?.[0] || (isEnglish ? 'Reference unavailable' : '暂无法比较')}</span></div>}{item.previous && <p className="growth-change">{changeLabel(latest, item.previous, locale)}</p>}</button></article>
}
...
function RecentGrowthRecords({ locale, evaluations, onOpenHistory }) {
  const isEnglish = locale === 'en-US'
  ...
  return <section className="growth-board-card growth-recent-records"><header className="growth-card-heading"><div><p className="eyebrow">{isEnglish ? 'Recent records' : '最近记录'}</p><h2>{isEnglish ? 'Facts remain traceable' : '事实保留可追溯'}</h2></div><button type="button" className="text-button" onClick={onOpenHistory}>{isEnglish ? 'View all' : '查看全部'}<ArrowRight size={15} /></button></header>{sorted.length ? <div className="growth-recent-list">{sorted.map((item) => <article key={item.id}><strong>{item.value} {item.unit}</strong><span>{metricLabel(item.type, locale)} · {formatDate(item.measuredAt, locale)}</span><small>{growthSourceLabel(item.source, locale)}{item.evaluation?.dataQuality === 'sufficient' ? ` · ${standardResultDetail(item.evaluation, locale)}` : ''}</small></article>)}</div> : <p className="growth-empty-note">{isEnglish ? 'No growth measurement yet.' : '还没有成长测量。'}</p>}</section>
}
...
function GrowthDataTable({ locale, model }) {
  const isEnglish = locale === 'en-US'
  return <section className="growth-chart-table"><header><strong>{isEnglish ? 'Measurement details' : '测量明细'}</strong><span>{isEnglish ? 'Fact and reference stay separate' : '事实与标准分开显示'}</span></header>{model.points.length ? <div className="growth-chart-table-scroll"><table><thead><tr><th>{isEnglish ? 'Date' : '日期'}</th><th>{isEnglish ? 'Value' : '数值'}</th><th>{isEnglish ? 'Reference' : '参考'}</th><th>{isEnglish ? 'Source' : '来源'}</th></tr></thead><tbody>{model.points.map((point) => <tr key={point.id}><td>{formatDate(point.measuredAt, locale)}</td><td>{point.value} {model.unit}</td><td>{point.conflicted ? (isEnglish ? 'Verify' : '需核对') : point.quality === 'standard' ? `${standardResultLabel(point.evaluation, locale)} · ${standardResultDetail(point.evaluation, locale)}` : (isEnglish ? 'Fact only' : '仅事实')}</td><td>{growthSourceLabel(point.source || point.evaluation?.measurementSource, locale)}</td></tr>)}</tbody></table></div> : <p className="growth-empty-note">{isEnglish ? 'No measurements in this range.' : '这个月龄范围内还没有测量。'}</p>}</section>
}
