...
export function CareOverview({ baby = null, careEvents = [], concerns = [], locale = 'zh-CN' }) {
  ...
  const legacySnapshot = getCareSnapshot(careEvents, concerns)
  ...
  const openConcerns = stateSnapshot.activeProblems.length ? stateSnapshot.activeProblems : legacySnapshot.openConcerns
  const missingBaselines = Object.values(stateSnapshot.baseline).filter((item) => item?.dimension && item.status === 'missing')
  return <section className="care-overview inspector-block" data-testid="care-overview">
    ...
    <div className="care-overview-last"><span>{isEnglish ? 'Last feed' : '最近喂养'}</span><strong>{eventTitle(legacySnapshot.lastFeeding, locale)}</strong><small>{formatEventTime(legacySnapshot.lastFeeding, locale)}</small></div>
    <div className="care-overview-last"><span>{isEnglish ? 'Last diaper' : '最近尿便'}</span><strong>{eventTitle(legacySnapshot.lastDiaper, locale)}</strong><small>{formatEventTime(legacySnapshot.lastDiaper, locale)}</small></div>
    ...
    {(stateSnapshot.current.conflicts.length > 0 || stateSnapshot.current.changes.length > 0 || missingBaselines.length > 0 || stateSnapshot.current.unknown.length > 0) && <div className="care-state-summary"><strong>{isEnglish ? 'Calculated from records' : '根据记录自动整理'}</strong>{stateSnapshot.current.conflicts.slice(0, 1).map((conflict) => <p key={conflict.id}>{isEnglish ? 'Conflicting caregiver observations need confirmation.' : conflict.message}</p>)}{stateSnapshot.current.changes.slice(0, 2).map((change) => <p key={change.id}>{isEnglish ? `${change.dimension}: ${change.status.replaceAll('-', ' ')}` : change.message}</p>)}{missingBaselines.slice(0, 1).map((baseline) => <p key={`baseline-${baseline.dimension}`}>{isEnglish ? `${baseline.dimension}: personal baseline is not established yet.` : `${baseline.dimension === 'feeding' ? '喂养' : '尿便'}暂无个人基线，当前只显示已记录事实。`}</p>)}{stateSnapshot.current.unknown.length > 0 && <p>{isEnglish ? `${stateSnapshot.current.unknown.length} domains have no current fact.` : `还有 ${stateSnapshot.current.unknown.length} 个领域尚未记录当前事实。`}</p>}</div>}
