...
export const HIGH_PRIORITY_FACTS = Object.freeze([
...
export const SUPPORT_SOURCE = Object.freeze({
  id: 'who-newborn-care',
  label: { zh: 'WHO：新生儿照护', en: 'WHO: Caring for a newborn' },
...
export function topicById(topicId) {
  return findTopicById(topicId) || SUPPORT_TOPICS.at(-1)
}

export function evaluateSupport({ topicId, facts = [] } = {}) {
  const topic = topicById(topicId)
  const urgent = facts.some((fact) => HIGH_PRIORITY_FACTS.some((item) => item.id === fact))
  if (urgent) {
    return {
      caregiverGuidance: 'immediate-contact',
  ...
  if (topic.id === 'temperature' || topic.id === 'breathing' || topic.id === 'alertness') {
    return {
      caregiverGuidance: 'prompt-contact',
  ...
  return {
    caregiverGuidance: 'observe-and-recheck',
...
export function concernsFromCareEvents(events = [], existing = []) {
  ...
  for (const event of events) {
    ...
    const unchanged = prior
      ...
      && prior.topicId === next.topicId
      && JSON.stringify(prior.plan || null) === JSON.stringify(next.plan || null)
      && JSON.stringify(prior.facts || []) === JSON.stringify(next.facts || [])
      && prior.notes === next.notes
    if (!unchanged) byId.set(concernId, next)
  }
  return [...byId.values()]
}
