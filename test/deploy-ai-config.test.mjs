...
test('Cloudflare deployment publishes the default LLM configuration before Pages', async () => {
  ...
  for (const name of ['OPENAI_API_KEY', 'OPENAI_BASE_URL', 'OPENAI_MODEL', 'OPENAI_USE_RESPONSES', 'LLM_KEY_ENCRYPTION_KEYS', 'LLM_KEY_ENCRYPTION_KEY_VERSION']) {
  ...
  assert.match(workflow, /node scripts\/validate-llm-keyring\.mjs/)
  assert.match(workflow, /\/api\/ai\/key-migration/)
  assert.ok(workflow.indexOf('pages deploy dist') < workflow.indexOf('/api/ai/key-migration'), 'plaintext migration must run against the newly deployed Functions code')
...
test('local secret sync preserves exact values without PowerShell pipeline newlines', async () => {
  ...
  assert.match(script, /'LLM_KEY_ENCRYPTION_KEYS'/)
