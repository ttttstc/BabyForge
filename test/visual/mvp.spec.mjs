...
test('birth measurements persist through onboarding and growth profile settings', async ({ page }) => {
  ...
  await expect(page.locator('.growth-state-summary strong').first()).toContainText('矫正年龄')
  ...
  await page.getByRole('button', { name: '保存事实' }).click()
  await page.getByRole('button', { name: '阶段', exact: true }).click()
  await expect(page.locator('.growth-state-summary strong').nth(1)).toContainText('2.4 kg')
  await page.getByRole('button', { name: '记录', exact: true }).click()
...
test('guest account can view the workspace but cannot edit records', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '记录', exact: true }).click()
  await page.locator('.record-more-card').filter({ hasText: '基础信息' }).click()
  await expect(page.getByLabel('年龄口径')).toBeDisabled()
