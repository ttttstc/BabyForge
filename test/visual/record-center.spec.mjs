...
async function createBaby(page) {
  ...
  await page.getByLabel('家庭名称').fill('历史日期家庭')
  await page.getByLabel('宝宝昵称').fill('小舟')
  await page.getByLabel('出生日期').fill(dateDaysAgo(10))
...
test('record center keeps six P0 facts and secondary records together', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '保存事实' }).click()
  await expect.poll(() => page.evaluate(() => JSON.parse(localStorage.getItem('babyforge:workspace:visual-test-user') || '{}').questions)).toEqual(['需要复测胆红素吗？', '吃奶量如何记录？'])
  ...
  const savedWorkspace = await page.evaluate(() => JSON.parse(localStorage.getItem('babyforge:workspace:visual-test-user')))
...
test('historical date becomes the P0 default and remains selected after save and correction', async ({ page }) => {
  ...
  await page.locator('.record-card').filter({ hasText: '喂奶' }).click()
  await page.getByTestId('record-entry-feeding').getByRole('button', { name: '配方奶' }).click()
  await expect(page.getByLabel('发生时间')).toHaveValue(new RegExp(`^${historicalDay}T`))
  await page.getByLabel('实际喝下奶量').fill('40')
  await page.getByRole('button', { name: '保存事实' }).click()
  await expect(page.locator('input[type="date"]').first()).toHaveValue(historicalDay)

  const original = await page.evaluate(() => JSON.parse(localStorage.getItem('babyforge:workspace:visual-test-user') || '{}').careEvents.find((event) => event.category === 'bottle_feeding'))
  await page.getByRole('button', { name: /纠正 配方奶 40 mL/ }).click()
  await expect(page.getByLabel('发生时间')).toHaveValue(new RegExp(`^${historicalDay}T`))
  await page.getByLabel('实际喝下奶量').fill('45')
  await page.getByRole('button', { name: '保存事实' }).click()
  await expect(page.locator('input[type="date"]').first()).toHaveValue(historicalDay)
  ...
  await expect(page.getByLabel('结束时间')).toHaveValue(new RegExp(`^${historicalDay}T`))
  await page.getByRole('button', { name: '保存事实' }).click()
  await page.locator('.record-card').filter({ hasText: '尿布' }).click()
  await expect(page.getByTestId('record-entry-diaper').getByLabel('发生时间')).toHaveValue(new RegExp(`^${historicalDay}T`))
  await page.getByTestId('record-entry-diaper').getByRole('button', { name: '只有尿' }).click()
  await page.locator('.record-card').filter({ hasText: '用药' }).click()
  await expect(page.getByLabel('发生时间')).toHaveValue(new RegExp(`^${historicalDay}T`))
  await page.getByLabel('药品名称').fill('历史药物')
  await page.getByRole('button', { name: '保存事实' }).click()
  await page.locator('.record-card').filter({ hasText: '体温' }).click()
  await expect(page.getByTestId('record-entry-temperature').getByLabel('测量时间')).toHaveValue(new RegExp(`^${historicalDay}T`))
  await page.getByRole('button', { name: '保存事实' }).click()
  await page.locator('.record-card').filter({ hasText: '成长测量' }).click()
  await expect(page.getByTestId('record-entry-growth').getByLabel('测量日期')).toHaveValue(historicalDay)
  await page.getByTestId('record-entry-growth').getByLabel('数值').fill('3.4')
  await page.getByRole('button', { name: '保存事实' }).click()
  await expect(page.locator('input[type="date"]').first()).toHaveValue(historicalDay)
})
