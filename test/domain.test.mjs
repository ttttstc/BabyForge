...
import { onRequestGet as onPhotoList, onRequestPost as onPhotoPost } from '../functions/api/photos.js'
...
import { MAX_PHOTO_BYTES, dateTimeInputToIso, detectPhotoTime, isSupportedPhoto, sortBabyPhotos } from '../src/domain/babyAlbum.js'
import { photoObjectKeys, photoVariantUrls, readPhotoAsset } from '../functions/_shared/photoVariants.js'
...
test('daily health reminders are age-aware and preserve today completion', () => {
  ...
  assert.equal(newborn.nutrition[0].id, 'nutrition-month-1')
  assert.equal(newborn.routine[0].id, 'routine-month-1')
  assert.equal(newborn.care, undefined)
  assert.equal(newborn.development, undefined)
  const completed = getDailyHealthReminders([{ taskId: 'nutrition-month-1', date: '2026-08-07', status: 'done' }], 6, date)
...
test('album sorts by photo time, then upload time and id, newest first', () => {
  const photos = [
    { id: 'old-upload', takenAt: '2026-08-20T08:00:00Z', createdAt: '2026-08-20T10:00:00Z' },
    { id: 'new-upload', takenAt: '2026-08-20T08:00:00Z', createdAt: '2026-08-20T11:00:00Z' },
    { id: 'recent-photo', takenAt: '2026-08-20T09:00:00Z', createdAt: '2026-08-19T11:00:00Z' },
  ]
  assert.deepEqual(sortBabyPhotos(photos).map((photo) => photo.id), ['recent-photo', 'new-upload', 'old-upload'])
  assert.deepEqual(photos.map((photo) => photo.id), ['old-upload', 'new-upload', 'recent-photo'])
})

test('cloud album list bounds rows and supports captured-time ranges', async () => {
  const parent = { token: 'token', expires_at: '2099-01-01T00:00:00.000Z', id: 'account-1', username: 'parent', role: 'admin', display_name: '家长' }
  const baby = { id: 'baby-1', householdId: 'household-1', nickname: '小舟', status: 'active' }
  const rows = Array.from({ length: 5 }, (_, index) => ({
    ...
    size_bytes: 1024,
    taken_at: `2026-08-${String(20 - index).padStart(2, '0')}T00:00:00.000Z`,
    time_source: 'exif',
  ...
  const env = {
    DB: {
      prepare: (sql) => ({
        bind: (...args) => ({
          first: async () => {
            statements.push({ sql, args, method: 'first' })
            if (sql.includes('auth_sessions')) return parent
            if (sql.includes('FROM baby_profiles')) return baby
            return null
          },
          all: async () => {
            statements.push({ sql, args, method: 'all' })
            return { results: rows.slice(0, Number(args.at(-1))) }
          },
        }),
      }),
    },
  ...
  const request = new Request('https://babyforge.test/api/photos?babyId=baby-1&limit=2&from=2026-08-19T00:00:00.000Z&to=2026-08-21T00:00:00.000Z', { headers: { cookie: 'babyforge_session=token' } })
  const response = await onPhotoList({ request, env })
  assert.equal(response.status, 200)
  const payload = await response.json()
  assert.equal(payload.photos.length, 2)
  assert.equal(payload.hasMore, true)
  const listStatement = statements.find((entry) => entry.method === 'all')
  assert.match(listStatement.sql, /taken_at >= \?/)
  assert.match(listStatement.sql, /taken_at < \?/)
  assert.match(listStatement.sql, /LIMIT \?/)
  assert.deepEqual(listStatement.args.slice(-1), [2])
})

test('photo variants use fixed private URLs and cache generated bytes in R2', async () => {
  const originalKey = 'babies/baby-1/photos/photo-1'
  const objects = new Map([[originalKey, { body: 'original-bytes' }]])
  let transforms = 0
  const env = {
    BABY_PHOTOS: {
      get: async (key) => objects.get(key) || null,
      put: async (key, bytes, options) => objects.set(key, { body: bytes, options }),
    },
    PHOTO_TRANSFORMER: {
      fetch: async (url) => {
        transforms += 1
        assert.match(url, /variant=thumb/)
        return new Response('thumb-bytes', { headers: { 'content-type': 'image/webp' } })
      },
    },
  }
  const first = await readPhotoAsset({ env, objectKey: originalKey, contentType: 'image/jpeg', variant: 'thumb' })
  assert.equal(await new Response(first.body).text(), 'thumb-bytes')
  assert.equal(transforms, 1)
  const second = await readPhotoAsset({ env, objectKey: originalKey, contentType: 'image/jpeg', variant: 'thumb' })
  assert.equal(transforms, 1)
  assert.equal(second.options.httpMetadata.contentType, 'image/webp')
  assert.deepEqual(photoVariantUrls('/api/photos/photo-1'), {
    thumbnailUrl: '/api/photos/photo-1?variant=thumb',
    displayUrl: '/api/photos/photo-1?variant=display',
  })
  assert.deepEqual(photoObjectKeys(originalKey), [originalKey, `${originalKey}/thumb.webp`, `${originalKey}/display.webp`])
})

test('concurrent photo variant reads share one transformer request', async () => {
  const originalKey = 'babies/baby-1/photos/photo-concurrent'
  let transforms = 0
  let release
  const gate = new Promise((resolve) => { release = resolve })
  const env = {
    BABY_PHOTOS: {
      get: async (key) => key === originalKey ? { body: 'original-bytes' } : null,
      put: async () => {},
    },
    PHOTO_TRANSFORMER: {
      fetch: async () => {
  ...
  const first = readPhotoAsset({ env, objectKey: originalKey, contentType: 'image/jpeg', variant: 'thumb' })
  const second = readPhotoAsset({ env, objectKey: originalKey, contentType: 'image/jpeg', variant: 'thumb' })
  await new Promise((resolve) => setTimeout(resolve, 0))
  assert.equal(transforms, 1)
  release()
  const [left, right] = await Promise.all([first, second])
  assert.equal(await new Response(left.body).text(), 'thumb-bytes')
  assert.equal(await new Response(right.body).text(), 'thumb-bytes')
})
...
test('guest sessions cannot delete album photos and authorized parents can', async () => {
  ...
  const parentEnv = {
    ...
    BABY_PHOTOS: { delete: async (keys) => { deletedKeys = keys } },
  }
  ...
  assert.deepEqual(deletedKeys, [
    'babies/baby-1/photos/photo-1',
    'babies/baby-1/photos/photo-1/thumb.webp',
    'babies/baby-1/photos/photo-1/display.webp',
  ])
...
test('authorized photo variants are transformed once and returned as private WebP', async () => {
  const guest = { token: 'token', expires_at: '2099-01-01T00:00:00.000Z', id: 'account-baby', username: 'baby', role: 'guest', display_name: '游客' }
  const photoRow = { id: 'photo-1', object_key: 'babies/baby-1/photos/photo-1', file_name: '第一天.jpg', content_type: 'image/jpeg', baby_status: 'active' }
  let cachedKey = ''
  let transformed = 0
  const env = {
    DB: { prepare: (sql) => ({ bind: () => ({ first: async () => sql.includes('auth_sessions') ? guest : photoRow }) }) },
    BABY_PHOTOS: {
      get: async (key) => key.endsWith('/thumb.webp') ? null : { body: 'photo-bytes' },
      put: async (key) => { cachedKey = key },
    },
    PHOTO_TRANSFORMER: {
      fetch: async () => {
        transformed += 1
  ...
  const request = new Request('https://babyforge.test/api/photos/photo-1?variant=thumb', { headers: { cookie: 'babyforge_session=token' } })
  const response = await onPhotoGet({ request, env, params: { id: 'photo-1' } })
  assert.equal(response.status, 200)
  assert.equal(response.headers.get('content-type'), 'image/webp')
  assert.equal(response.headers.get('cache-control'), 'private, max-age=604800')
  assert.equal(await response.text(), 'thumbnail')
  assert.equal(transformed, 1)
  assert.equal(cachedKey, 'babies/baby-1/photos/photo-1/thumb.webp')
})
