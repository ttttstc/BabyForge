...
打开终端显示的本地地址。完整的本地登录由 Pages Functions 提供，预置身份仅通过未入库的 `BABYFORGE_PRESET_ACCOUNTS` 运行时 Secret 配置：

- 通用 Mock 演示：宝宝资料不含任何特定家庭身份。
- 品牌演示：只读展示 Secret 指定宝宝的脱敏资料与真实照片。
- 正式管理员：账号、邮箱和既有家庭资源映射均由服务端 Secret 指定。
- 演示边界：中性 Mock 仅使用浏览器内虚构数据；品牌演示只通过专用短期 Session 读取 allowlist 资料与照片，不能写入，也不访问邮件、Google、AI 对话或配置。

## 演示用例数据

使用已配置的品牌演示账号登录后，可只读浏览经过字段过滤的展示资料与照片；中性 Mock 账号提供不含特定家庭身份的虚构档案。展示内容不代表医疗建议：

- 宝宝档案：泥蛙；示例身长 `50 cm`、体重 `3 kg`；混合喂养；可查看出生档案、成长年龄和记录人信息。
- 今天：查看亲喂、瓶喂、睡眠、尿布、体温等照护事实的当日汇总，并从汇总卡进入对应记录。
- 记录：按日期和类型浏览 `CareEvent` 事实时间线，打开事实详情、来源和版本历史。
- 成长：查看身长 `50 cm`、体重 `3 kg` 等测量、个人变化、百分位参考和数据表。
- 健康：打开 0–6 岁疫苗路线、儿科主题、教育病例和器官教学；3D 不可用时会自动降级为 2D。
- 经验与奶爸 AI：浏览适龄内容、就医/照护交接摘要和 AI 功能边界；需要服务端密钥的内容按部署配置显示。
