...
- 演示边界：仅使用浏览器内虚构数据；不访问生产 D1、R2、邮件、Google 或 AI Secret，刷新或退出即重置。

## 演示用例数据

使用已配置的品牌演示账号登录后，可按下面这份虚构档案浏览产品能力；中性 Mock 账号提供同等完整但不含特定家庭身份的信息。数据仅用于界面展示，不代表真实医疗建议，也不包含真实家庭信息：
