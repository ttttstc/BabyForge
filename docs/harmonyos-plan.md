...
- Vite production build：通过；业务路由改为首屏静态导入，导航不再等待路由 chunk，入口 JS gzip 约 376 KB（低于 400 KB 门槛）；3D 引擎仍保持独立 chunk 并在进入模型区域时按需加载。本轮未执行 Cloudflare 生产部署，线上资源仍返回旧的 `rememberMe: false` 构建，因此 H-04 会话恢复尚未闭环。
