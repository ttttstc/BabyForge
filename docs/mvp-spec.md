# BabyForge 新生儿—黄疸 MVP Spec

版本：0.1  
状态：研究原型  
范围：出生后 0–28 天、单宝宝、本地数据、中文界面
...
1. 家长创建宝宝档案。
2. 系统按出生日期显示新生儿早期或适应期。
3. 家长在三栏工作台看到今日三项重点。
4. 家长通过五步 2D/3D 场景理解黄疸位置和机制。
5. 家长保存原始观察，生成可复制、可打印的就医沟通摘要。
...
| 路由 | 页面 | 关键行为 |
| --- | --- | --- |
| `#/onboarding` | 首次建档 | 昵称、出生日期、孕周、喂养方式 |
| `#/today` | 今天工作台 | 日龄、当前阶段、三项重点、主题入口 |
| `#/stage/newborn` | 新生儿阶段 | 阶段教育示意、2D/3D 切换 |
| `#/topic/jaundice` | 黄疸专题 | 五步场景、观察表、来源与安全门 |
| `#/doctor-summary` | 就医摘要 | 原始记录、provenance、复制与打印 |

不存在宝宝档案时，任何路由都返回建档；建档完成后进入今天页。
...
- 29 天起：`out-of-scope`，只提示超出研究范围，不推导新阶段。
- 未来出生日期为非法输入。
...
Key：`babyforge:workspace`，版本：2。保存宝宝、观察记录、咨询问题、2D/3D 与性能偏好。`sex` 使用 `male | female`；旧档案缺失该字段时迁移为 `null` 并继续显示中性占位，不猜测性别。解析失败、未知版本或清除操作均回到空白初始状态。
...
- Playwright 覆盖男女建档与刷新持久化、黄疸五步、2D 降级、记录、摘要、清除数据、移动抽屉。
...
收到按 `prompt/` 生成的 PNG/GLB 后，单独执行资产接入：检查几何与材质、生成/验证低模、填写节点映射、打开 manifest `ready`、测试加载性能并更新最终视觉快照。其他疾病、后端和临床规则均另立任务。
