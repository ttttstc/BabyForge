...
check('原生 OAuth 必须使用一次性令牌交换并注册回调深链', nativeAdapter.includes('/api/native/auth/callback') && nativeAdapter.includes('/api/auth/one-time-token/verify') && nativeAuthCallback.includes('generateOneTimeToken') && nativeAuthCallback.includes('babyforge://auth/callback') && moduleConfig.includes('"scheme": "babyforge"') && moduleConfig.includes('"host": "auth"') && ability.includes('onNewWant') && betterAuthConfig.includes('oneTimeToken'))
